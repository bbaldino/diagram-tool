...
describe('layoutDiagram dispatcher', () => {
  const diagram = {
    ...
    nodes: [
      { id: 'in', label: 'In', fields: [], position: { x: 0, y: 0 }, parentId: 'g' },
      {
        id: 'out',
        label: 'Out',
        fields: [{ key: 'host', value: 'x', showOnNode: true }],
        position: { x: 0, y: 0 },
      },
    ],
    groups: [
  ...
  for (const engine of ['elk', 'graphviz'] as const) {
    it(`(${engine}) returns child positions relative to the group and preserves node fields`, async () => {
      ...
      expect(nodes.find((n) => n.id === 'out')!.fields).toEqual([
        { key: 'host', value: 'x', showOnNode: true },
      ]) // preserved
    })
...
describe('handlesFor', () => {
  ...
  it('auto: target to the right → source right, target left', () => {
    expect(handlesFor('auto', S, { x: 300, y: 0 })).toEqual({
      sourceHandle: 'right',
      targetHandle: 'left',
    })
  })
  ...
  it('auto: target below → source bottom, target top', () => {
    expect(handlesFor('auto', S, { x: 0, y: 300 })).toEqual({
      sourceHandle: 'bottom',
      targetHandle: 'top',
    })
  })
  ...
  it('auto: exact tie (|dx| == |dy|) resolves horizontal', () => {
    expect(handlesFor('auto', S, { x: 100, y: 100 })).toEqual({
      sourceHandle: 'right',
      targetHandle: 'left',
    })
  })
  it('undefined orientation behaves like auto', () => {
    expect(handlesFor(undefined, S, { x: 300, y: 0 })).toEqual({
      sourceHandle: 'right',
      targetHandle: 'left',
    })
  })
...
describe('layoutDiagram edge handles', () => {
  ...
  it("absoluteCenter: grouped node center adds the parent group's absolute position back onto the parent-relative placement", () => {
    ...
    expect(center).toEqual({
      x: withoutParent.x + group.position.x,
      y: withoutParent.y + group.position.y,
    })
  })
